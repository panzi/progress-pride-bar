...
fn main() {
    ...
    if let Some(Duration (animation_duration)) = args.animate {
        ...
        for i in istart..=steps {
            ...
            let f = if args.reverse { steps - i } else { i } as f64;
            let _ = print_progress_pride_bar(&mut out, width, f / steps as f64, args.background);
            let _ = write!(out, "\x1B[0m");
            let _ = out.flush();

            let sleep_duration = animation_duration * (i + 1 - istart) / steps;
            let elapsed = animation_start_ts.elapsed();
            if sleep_duration > elapsed {
                ok = interruptable_sleep(sleep_duration - elapsed);
                if !ok {
                    break;
                }
            }
        ...
        let _ = write!(out, "\x1B[0m\x1B[?25h\x1B[?7h\n");
    } else {
        let width = args.width();
